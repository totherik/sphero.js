 ...
 * @param options (optional) options for the current command: resetTimer (boolean) and requestAck (boolean)
...
function command(did, cid, encode, decode, options, callback) {
    ...
    options.resetTimer = (options.resetTimer !== undefined) ? options.resetTimer : this.resetTimer;
    ...
    if (options.requestAck) {
        packet.seq = this._add(function (err, result) {
            if (err) {
                err.did = packet.did;
                err.cid = packet.cid;
                err.seq = packet.seq;
    ...
    return {
        ...
        createRobot: function (options) {
            ...
            robot = Object.create(proto, {
                ...
                resetTimer: {
                    value: options.resetTimer || false,
                    enumerable: true,
                    writable: true
                },
            ...
            robot.on('packet', robot._remove.bind(robot));
            return robot;
        }
